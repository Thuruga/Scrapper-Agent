...
class NetshoesShipping(BaseShipping):
    ...
    async def calculate(
        ...
        try:
            ...
            logger.warning(
                "[NetshoesShipping] quote failed brand=%s status=%s",
        ...
        if not result:
            # D-01: o engine nao distingue bloqueio Akamai de "sem resultado" — a
            # realidade documentada e um bloqueio permanente de infra, entao um
            # resultado None cai conservadoramente em BLOCKED (nunca em frete 0.0/gratis).
            return ShippingCalculation(
                state=ShippingState.BLOCKED,
                message=DEFAULT_MESSAGES[ShippingState.BLOCKED],
            )
