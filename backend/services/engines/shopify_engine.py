...
from core.models import ShippingInfo
...
class ShopifyEngine(BaseEngine):
    ...
    async def search(
        ...
        if include_shipping and result and hasattr(result, 'products'):
            for product in result.products:
                product.shipping = ShippingInfo(status="Calculado no checkout")
    ...
    async def calculate_shipping(self, product: Any, zipcode: str) -> Optional[Dict[str, Any]]:
        # Shopify shipping calculation not implemented for now
        return None
